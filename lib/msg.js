...
export async function smsg(conn, m) {
    ...
    if (m.message) {
        const msgKeys = Object.keys(m.message).filter(k => k !== 'messageContextInfo');
        m.mtype = msgKeys[0] || Object.keys(m.message)[0];
        ...
        if (quotedRaw) {
            ...
            if (!realQuoted) {
                ...
                let quotedKeys = Object.keys(realQuoted).filter(k => k !== 'messageContextInfo');
                let type = quotedKeys[0] || Object.keys(realQuoted)[0];
                ...
                if (!quotedContent) {
                    ...
                    quotedObj.fakeObj = { key: { remoteJid: m.msg.contextInfo?.remoteJid || m.chat, fromMe: false, id: m.msg.contextInfo?.stanzaId || null, participant: m.msg.contextInfo?.participant || null }, message: { [type]: quotedContent } };
