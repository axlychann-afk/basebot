...
const generateMessageIDV2 = () => crypto.randomBytes(16).toString('hex').toUpperCase();
...
class Toolkit {
    ...
    static async toUrl(_iki, path, mediaType = 'document') {
    ...
    static async resolveMedia(_iki, media, mediaType = 'image', { resolveUrl = false, resolveWAUrl = false, result = 'url', resize = false, width = 300, height = 300 } = {}) {
        ...
        if (Array.isArray(media)) {
            return Promise.all(
                media.map((item) =>
                    Toolkit.resolveMedia(_iki, item, mediaType, {
                        resolveUrl,
                        resolveWAUrl,
                        result,
                        resize,
        ...
        if (originalIsBuffer) {
            return Toolkit.toUrl(_iki, media, mediaType);
        }
...
class AIRich extends BaseBuilder {
    ...
    constructor(iki, { dynamic = true, unsupportedTypeAlert = true } = {}) {
        if (!iki) {
    ...
    addText(text, { hyperlink = true, citation = true, latex = true, id, replace, insertAt } = {}) {
        ...
        const { text: extractedText, inline_entities } = Toolkit.extractIE(text, {
            hyperlink,
            citation,
            latex,
        });
    ...
    addSource(sources = [], { id, replace, insertAt } = {}) {
        ...
        const source = normalizedSources.map(({ icon, url, title, subtitle }) => ({
            ...
            source_url: url,
            favicon: {
                url: Toolkit.resolveMedia(this.#iki, icon, 'image'),
    ...
    addReels(reelsItems = [], { id, replace, insertAt } = {}) {
        ...
        const reels = items.map((item) => ({
            ...item,
            _avatar: Toolkit.resolveMedia(this.#iki, item.profileIconUrl ?? item.profile_url ?? item.profile ?? '', 'image'),
            _thumbnail: Toolkit.resolveMedia(this.#iki, item.thumbnailUrl ?? item.thumbnail ?? '', 'image'),
        }));
    ...
    addImage(imageUrl, { width, height, status = 'READY', update_text, resolveUrl = false, id, replace, insertAt } = {}) {
        ...
        const list = Array.isArray(imageUrl)
            ? imageUrl.map((v) => {
                    const url = Toolkit.resolveMedia(this.#iki, v, 'image', { resolveUrl });
            ...
            : (() => {
                    const url = Toolkit.resolveMedia(this.#iki, imageUrl, 'image', { resolveUrl });
    ...
    addVideo(videoUrl, { autoFill = true, status = 'READY', estimatedTime, id, replace, insertAt } = {}) {
        ...
        for (const item of items) {
            ...
            const url = isObject ? Toolkit.resolveMedia(this.#iki, item.url ?? '', 'video') : Toolkit.resolveMedia(this.#iki, item, 'video');
            ...
            const thumbnail =
                isObject && item.thumbnail
                    ? Toolkit.resolveMedia(this.#iki, item.thumbnail, 'image', {
                            result: 'base64',
                            resize: true,
                            width: 300,
                            height: 300,
    ...
    addProduct(data = {}, { id, replace, insertAt } = {}) {
        ...
        const product = items.map((item) => ({
            ...
            image: {
                url: Toolkit.resolveMedia(this.#iki, item.image_url ?? item.image, 'image'),
            },
            additional_images: [
                {
                    url: Toolkit.resolveMedia(this.#iki, item.icon_url ?? item.icon, 'image'),
                },
            ],
    ...
    addPost(data = {}, { id, replace, insertAt } = {}) {
        ...
        const primitives = posts.map((p) => ({
            ...
            subtitle: p.subtitle ?? '',
            username: p.username ?? '',
            profile_picture_url: Toolkit.resolveMedia(this.#iki, p.profile_picture_url ?? p.profile_url ?? p.profile ?? '', 'image'),
            is_verified: !!(p.is_verified || p.verified),
            thumbnail_url: Toolkit.resolveMedia(this.#iki, p.thumbnail_url ?? p.thumbnail ?? '', 'image'),
            ...
            footer_icon: Toolkit.resolveMedia(this.#iki, p.footer_icon ?? p.icon ?? '', 'image'),
    ...
    async build(
        ...
        const sections = this._footer
            ? [
                    ...(await Toolkit.waitAllPromises(this._sections)),
                    AIRich.newLayout('Single', {
                        text: this._footer,
                        __typename: 'GenAIMetadataTextPrimitive',
                    }),
            ...
            jid,
            {
                ...
                botForwardedMessage: {
                    message: {
                        richResponseMessage: {
                            messageType: 1,
                            submessages: includesSubmessages ? await Toolkit.waitAllPromises(this._submessages) : [],
                            unifiedResponse: {
    ...
    async sendEdit(jid, id, { msg, messageId, additionalNodes = [], ...options } = {}) {
        ...
        await this.#iki.relayMessage(jid, msgEdit.message, {
    ...
    async send(jid, { bypassDownload = true, forwarded = true, notification = false, includesUnifiedResponse = true, includesSubmessages = true, messageId, additionalNodes = [], ...options } = {}) {
        ...
        await this.#iki.relayMessage(msg.key.remoteJid, msg.message, {
    ...
    static toTableMetadata(arr, { hyperlink = true, citation = true, latex = true } = {}) {
        ...
        const unified_rows = [
            ...
            const markdown_cells = row.cells.map((cell) => {
                const extracted = Toolkit.extractIE(cell, { hyperlink, citation, latex });
    ...
    async toCard() {
        return {
            ...
            header: {
                ...
                ...(this._data
                    ? await prepareWAMessageMedia(this._data, { upload: this.#iki.waUploadToServer }).catch((e) => {
