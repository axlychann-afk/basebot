...
import { bindGroupNotify } from './lib/groupNotify.js'
import { bindStatusAutoReact } from './lib/statusAutoReact.js'
import { bindReactionActions } from './lib/reactionActions.js'
...
async function start() {
    ...
    try {
        ...
        bind(socket)
        bindGroupNotify(socket)
        bindStatusAutoReact(socket)
        bindReactionActions(socket)
            ...
            for (const rawM of messages) {
                setImmediate(async () => {
                    try {
                        ...
                        if (!m.key.fromMe) {
                            socket.readMessages([m.key]).catch(() => {})
                        }
