...
async function loadPlugin(file) {
    try {
        ...
        const keys = []
        if (!handler.category) {
            handler.category = path.relative(pluginDir, path.dirname(file)) || 'lainnya'
        }
...
export default async function handleMessage(conn, m) {
    try {
        ...
        const isGroupAdmin = async () => {
            ...
            try {
                ...
                const participant = meta?.participants?.find(p => (p.id || '').split('@')[0].split(':')[0] === senderNum)
        ...
        const checkAccess = async handler => {
            ...
            if (handler.admin) {
                permissions.push(['admin', await isGroupAdmin(), config.accessDenied.admin])
            }
    ...
    showTyping()
    const denied = await checkAccess(handler)
    if (denied) return
